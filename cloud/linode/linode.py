...
options:
  ...
  additional_disks:
    description:
     - > List of dictionaries for creating additional disks that are added to the Linode configuration settings. Dictionary uses: Size, Label, Type. Size is in MB.
    default: null
    version_added: "2.2"
  alert_bwin_enabled:
    description:
    ...
    if state in ('active', 'present', 'started'):
        ...
        if not servers:
            ...
            new_server = True
             ...
             # Get size of all individually listed disks to subtract from
             # Distribution disk
